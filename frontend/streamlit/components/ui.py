...
def render_course_card(title, description, topics, accent=''):
    gradient_style = f"background: linear-gradient(to right, {accent}); -webkit-background-clip: text; -webkit-text-fill-color: transparent;" if accent else ''
    topic_html = ''.join([f"<span style='display: inline-flex; border-radius: 999px; background: rgba(255, 255, 255, 0.05); padding: 0.5rem 0.75rem;'>{topic}</span>" for topic in topics])
    
    st.markdown(
        f"""
        <article class='glass-panel card p-8' style='height: 100%; display: flex; flex-direction: column; margin-bottom: 1.5rem;'>
            <div style='margin-bottom: 1rem; display: inline-flex; border-radius: 999px; padding: 0.5rem 0.75rem; font-size: 0.875rem; font-weight: 600; {gradient_style}'>
                {title}
            </div>
            ...
            <div style='margin-top: 1.5rem; display: flex; flex-wrap: wrap; gap: 0.5rem; font-size: 0.875rem; color: #e2e8f0;'>
                {topic_html}
            </div>
...
def render_practice_card(title, description, items, accent=''):
    ...
    st.markdown(
        f"""
        <article class='glass-panel card p-8' style='height: 100%; display: flex; flex-direction: column; margin-bottom: 1.5rem;'>
            <div style='display: inline-flex; border-radius: 999px; {gradient_style} padding: 0.5rem 0.75rem; font-size: 0.875rem; font-weight: 600; color: #ffffff; align-self: flex-start;'>
                {title}
            </div>
            <h2 style='margin-top: 1.5rem; font-size: 1.5rem; font-weight: 600; color: #ffffff; margin: 0;'>{title}</h2>
            <p style='margin-top: 0.75rem; color: #cbd5e1; flex-grow: 1;'>{description}</p>
            <ul style='margin-top: 1.5rem; padding: 0; list-style: none; color: #e2e8f0; margin-bottom: 0;'>
                {items_html}
            </ul>
        </article>
        """,
